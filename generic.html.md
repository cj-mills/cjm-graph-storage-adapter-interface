...
*Normalize a tagged wire dict to a `NodeQuery` (typed values pass
through).*
...
*Normalize a tagged wire dict to an `EdgeQuery` (typed values pass
through).*
...
*Normalize a tagged wire dict to a `RawQuery` (typed values pass
through).*
...
def GenericGraphStorageAdapter(
